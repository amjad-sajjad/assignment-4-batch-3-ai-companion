    ...
    return (
        <header className="flex items-center mb-12 justify-between">
            ...
            <ul className="ml-4 text-sm text-zinc-400 flex gap-8">
                <a  className="hover:text-zinc-200 font-medium text-zinc-200 cursor-pointer transition-all"
                onClick={()=>setPage("create") }>Create Image</a>
                <a  className="hover:text-zinc-200 cursor-pointer transition-all" onClick={()=>setPage("download")}>Downloaded</a>
            </ul>
        </header>
    );
};
